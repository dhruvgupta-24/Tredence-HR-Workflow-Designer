...
export function CanvasControls({
  onShortcutsOpen, onCopilotOpen, onCommandOpen, onDemoRun, isDemoRunning,
}: Props) {
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    importWorkflow(
      file,
      (n: WorkflowNode[], ed: Edge[]) => {
        setNodes(n); setEdges(ed); triggerFitView()
  ...
  return (
    <div className="h-12 flex-shrink-0 border-b border-gray-800/80 bg-gray-950 flex items-center px-3 gap-1">
      ...
      <div className="flex items-center gap-1">
        {/* AI Copilot */}
        <button type="button" onClick={onCopilotOpen} title="AI Workflow Copilot"
        ...
        {/* Guided Demo */}
        <button type="button" onClick={onDemoRun} disabled={isDemoRunning} title="Guided product tour"
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-violet-600/10 border border-violet-500/20 hover:bg-violet-600/20 hover:border-violet-500/40 transition-all duration-150 text-violet-400 hover:text-violet-300 disabled:opacity-35 disabled:pointer-events-none"
        >
      ...
      {/* Layout group */}
      <div className="flex items-center gap-0.5">
        <button type="button" onClick={autoArrange} disabled={nodes.length === 0} title="Auto-arrange layout"
          className="p-2 rounded-lg text-gray-500 hover:text-white hover:bg-gray-800 transition-all duration-150 disabled:opacity-20 disabled:pointer-events-none"
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <button type="button" onClick={optimize} disabled={nodes.length === 0} title="Optimize workflow"
      ...
      {/* Command palette trigger - product-grade SaaS style */}
