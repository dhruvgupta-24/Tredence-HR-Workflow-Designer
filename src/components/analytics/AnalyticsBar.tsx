...
function Sep() {
  return <span className="w-px h-3 bg-gray-800 flex-shrink-0" aria-hidden />
}
...
export function AnalyticsBar() {
  ...
  const stats = useMemo(() => {
    ...
    // Bottleneck = node with most in-edges (fan-in convergence point)
  ...
  return (
    <div
      className="h-9 flex-shrink-0 border-t border-gray-800/60 bg-gray-950/90 flex items-center px-4 gap-3 overflow-hidden"
      aria-label="Workflow analytics"
    >
      <Stat label="Nodes"      value={`${stats.nodeCount}`} />
      <Sep />
      <Stat label="Automation" value={`${stats.autoPct}%`}
        valueClass={stats.autoPct >= 40 ? 'text-green-400' : 'text-gray-300'} />
      <Sep />
      ...
      <Stat label="Approvals" value={`${stats.approvals}`}
        valueClass={stats.approvals >= 3 ? 'text-amber-400' : 'text-gray-300'} />
      ...
      {stats.approvals >= 3 && (
        <>
          <Sep />
          <span className="text-[10px] text-amber-600 font-medium">Long approval chain</span>
        </>
      )}

      <div className="flex-1" />
      <span className="text-[10px] text-gray-700 font-mono">{stats.edgeCount} connections</span>
    </div>
  )
}
